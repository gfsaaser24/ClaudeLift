...
function buildServer(): McpServer {
  ...
  server.registerTool(
    ...
    {
      title: 'Import a bundle back into Cowork',
      description:
        'Restore an exported bundle into the local Claude Cowork sandbox so the task reappears in ' +
        "Cowork's sidebar. Pass the bundle directory (must contain manifest.json — e.g. one from " +
        'claudelift_list_bundles). Runs a dry run by default: it reports any required folder remaps ' +
        '(the unmapped folder and the exact remap to supply) and, when several workspaces exist, the ' +
        'candidate workspaces to choose from — read them, then retry with the extra params. Set ' +
        'dry_run=false to actually import. Auth artefacts are always skipped and nothing from the ' +
        'bundle is deleted; a same-id target is refused unless force=true, which overwrites ' +
        '(permanently destroys) the existing task. Restart Claude Desktop afterward to see the ' +
        'imported task.',
      inputSchema: {
        bundle_dir: z.string().describe('Path to the bundle directory (contains manifest.json).'),
        dry_run: z
          .boolean()
          .default(true)
          .describe('Preview the plan without writing (default true). Set false to actually import.'),
        workspace: z
          .string()
          .optional()
          .describe('Destination <account>/<workspace> directory; required when several are detected (a dry run lists the candidates).'),
        remaps: z
          .array(z.object({ from: z.string(), to: z.string() }))
          .default([])
          .describe('Folder remaps — each manifest source_user_folders path mapped to a local path (a dry run reports which are required).'),
        keep_task_id: z
          .boolean()
          .default(false)
          .describe('Reuse the original task id instead of assigning a fresh one.'),
        force: z.boolean().default(false).describe('Overwrite an existing task with the same id.')
      },
      annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false }
    },
    async ({ bundle_dir, dry_run, workspace, remaps, keep_task_id, force }) => {
      const dir = bundle_dir.trim()
      if (dir.length === 0) return fail('bundle_dir is required.')
      try {
        const res = await importBundle({
          bundleDir: dir,
          workspace,
          remaps,
          keepTaskId: keep_task_id,
          dryRun: dry_run,
          force
        })
        const header = res.dryRun
          ? 'Dry run — no changes written. Review the plan below, then call again with dry_run=false to import. ' +
            'The new_task_id in the plan is only a preview — the real import assigns a fresh id unless keep_task_id=true.'
          : res.newTaskId !== null
            ? `Imported. New task id: ${res.newTaskId}\nRestart Claude Desktop to see it in the sidebar.`
            : 'Import completed.'
        return ok(`${header}\n\n${res.stdout.trim()}`)
      } catch (err) {
        return fail(`could not import bundle "${dir}": ${errText(err)}`)
      }
    }
  )
