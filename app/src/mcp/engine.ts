...
/**
 * Restore an exported bundle into the local Cowork sandbox. `--skip-auth`
 * is always passed (the MCP surface never touches auth artefacts). Nothing
 * from the bundle is deleted, and an existing same-id task is refused unless
 * `force` is set — which overwrites (destroys) it. Returns the parsed
 * `new_task_id` (always null on a dry run: the plan's id is only a preview and
 * the real import assigns a fresh one unless keep_task_id) and the engine's
 * stdout.
 *
 * On a non-zero exit the engine's FULL stderr is surfaced (not just the first
 * line): the "pick a --workspace" and "add a --remap" guidance the engine
 * emits carries the candidate workspaces / the unmapped folder + exact syntax
 * on later lines, and an MCP-only agent needs those to recover.
 */
export async function importBundle(
  opts: ImportOptions
): Promise<{ newTaskId: string | null; stdout: string; dryRun: boolean }> {
  const dir = resolve(opts.bundleDir)
  if (!existsSync(join(dir, 'manifest.json'))) {
    throw new Error(`no bundle at ${dir} — a bundle directory must contain manifest.json`)
  }
  // The engine parses each --remap token by splitting on the FIRST '=', so a
  // source path containing '=' can't be expressed as from=to (it mis-splits
  // into the wrong `from` and can never match). Reject it up front with a
  // clear message instead of an unsatisfiable, self-contradictory engine error.
  for (const r of opts.remaps ?? []) {
    if (r.from.includes('=')) {
      throw new Error(
        `remap source path contains '=', which the from=to encoding can't represent: ${r.from}. ` +
          'Import this bundle from the ClaudeLift desktop app instead.'
      )
    }
  }
  const args = ['import', dir, '--skip-auth']
  if (opts.workspace !== undefined && opts.workspace.length > 0) args.push('--workspace', opts.workspace)
  for (const r of opts.remaps ?? []) args.push('--remap', `${r.from}=${r.to}`)
  if (opts.keepTaskId) args.push('--keep-task-id')
  if (opts.dryRun) args.push('--dry-run')
  if (opts.force) args.push('--force')
  const result = await runEngine(args)
  if (result.code !== 0) {
    // Surface EVERY non-empty stderr line. The engine's actionable guidance
    // (candidate workspaces to pass as --workspace; the unmapped folder and
    // exact --remap syntax) lives on lines 2+, and an agent driving only this
    // tool needs it to correct the call and retry.
    const detail = fullStderr(result.stderr)
    throw new Error(`engine import failed (exit ${result.code})${detail ? `:\n${detail}` : ''}`)
  }
  // The plan prints a preview new_task_id, but the real import assigns a fresh
  // id (unless keep_task_id) — never report the dry-run preview as the created
  // task's id.
  const m = result.stdout.match(/^\s*new_task_id:\s*(\S+)/m)
  return {
    newTaskId: opts.dryRun === true ? null : m !== null ? m[1] : null,
...
/**
 * Every non-empty trimmed stderr line, joined. Import surfaces this (not just
 * the first line) so the engine's multi-line "pick a --workspace / add a
 * --remap" guidance reaches the agent. Bounded by STDERR_CAP upstream.
 */
function fullStderr(stderr: string): string {
  return stderr
    .split(/\r?\n/)
    .map((line) => line.trim())
