...
ClaudeLift ships a standalone **[Model Context Protocol](https://modelcontextprotocol.io) server** so any MCP-capable agent — **Claude Desktop, Claude Code, or Cursor** — can read and export your Cowork chats from the chat window. Ask *"pull the transcript of my SOP chat"* and the agent does it. It exposes five `claudelift_` tools (list tasks, get transcript, build a seed prompt, list bundles, export a task); four are read-only and export only writes bundle files — import, purge, and Notion stay in the app. It's a **local stdio** server that runs `ClaudeLift.exe` as its own Node runtime, so there's nothing extra to install once ClaudeLift is on disk.
